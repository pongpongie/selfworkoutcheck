...
| 🏆 | **개인 기록** | 종목 카드에 역대 최고 중량과 날짜를 띄운다 |
| 📈 | **중량 추이** | 최고 중량 · **추정 1RM** · 볼륨 세 지표로 본다. SVG를 직접 그린다 |
...
| ↕ | **순서 변경** | 루틴 안에서 종목 순서를 바꾼다 |
| 🏋️ | **원판 계산** | 한쪽 원판 구성을 그림으로. 봉 무게는 설정에서 바꾼다 (여성봉·EZ바) |
| 💡 | **훈련 팁** | 중량 올리는 기준·세트·기록 습관. 매 화면에 깔지 않고 따로 뺐다 |
| 🏠 | **홈 대시보드** | 가장 오래 안 한 루틴을 제안하고, 이번 주 빈도와 연속 일수를 보여준다 |
...
킷의 `container background` / `container border` 를 유리 표면으로 썼다.
...
**유리는 뒤에 뭔가 있어야 보인다.** 평평한 배경 위에서 blur 는 아무 일도 안 한다.
그래서 앰비언트를 먼저 깔았다. `body` 배경을 비우고 `html` 이 캔버스를 지는 이유도
같다 — `body` 를 칠하면 `body::before` 앰비언트가 가려진다.

`prefers-reduced-transparency: reduce` 면 불투명 표면으로 떨어진다.
...
- **iOS** — 공유 → "홈 화면에 추가". 사파리·크롬 모두 된다.
  카카오톡·인스타 같은 인앱 브라우저에서는 메뉴가 없을 수 있다
